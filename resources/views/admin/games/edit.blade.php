...
@section('title', "Editar {$game->title} — Administración ROMHUB")

@section('content')
<div class="space-y-6" x-data="gameEditForm()">
    ...
    <div class="flex items-center justify-between">
        <div class="space-y-1">
            ...
            <h1 class="text-2xl font-black text-white tracking-tight font-sans">Editar Videojuego</h1>
        </div>
    ...
    <form action="{{ route('admin.games.update', $game->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="title" x-model="title" required class="w-full bg-[#0A0C0F] border border-[#232936] rounded-xl p-3 text-sm text-white focus:outline-none focus:border-blue-500 font-sans">
                        </div>

                        <div>
                            <label class="block text-xs font-mono text-gray-400 mb-1">Slug URL</label>
                            <input type="text" name="slug" value="{{ old('slug', $game->slug) }}" class="w-full bg-[#0A0C0F] border border-[#232936] rounded-xl p-2.5 text-xs text-gray-300 font-mono">
                        </div>

                        <div>
                            <label class="block text-xs font-mono text-gray-400 mb-1">Consola / Plataforma *</label>
                            <select id="consoleSelect" name="console_id" required class="w-full bg-[#0A0C0F] border border-[#232936] rounded-xl p-2.5 text-xs text-gray-200 focus:outline-none focus:border-blue-500 font-sans">
                                <option value="">Selecciona la consola...</option>
                                @foreach($consoles as $con)
                                    <option value="{{ $con->id }}" {{ old('console_id', $game->console_id) == $con->id ? 'selected' : '' }}>
                                        {{ $con->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <!-- Rich Markdown Synopsis & Review Section -->
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-6 space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#232936] pb-3">
                        <div class="flex items-center gap-3">
                            <h2 class="text-sm font-bold text-white font-sans uppercase tracking-wider flex items-center gap-2">
                                <i data-lucide="book-open" class="w-4 h-4 text-emerald-400"></i> Sinopsis & Análisis Profundo
                            </h2>
                        ...
                        <button type="button" 
                            ...
                            <span x-text="descLoading ? 'Generando ~200 palabras...' : '✨ Regenerar Sinopsis IA (~200 palabras)'"></span>
                        </button>
                    </div>
                        ...
                        <div class="bg-[#11141A] border-b border-[#232936] px-3 py-2 flex flex-wrap items-center justify-between gap-2">
                            ...
                            <div x-show="editorTab === 'write'" class="p-2">
                                <textarea id="descriptionEditor" 
                                          ...
                                          x-model="description" 
                                          rows="9" 
                                          placeholder="Escribe aquí la historia, ambientación, mecánicas y legado del videojuego en ~200 palabras (o pulsa 'Regenerar con IA' para que Gemini la redacte por ti)..." 
                ...
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs font-mono">
                        <div>
                            ...
                            <input type="text" name="developer" x-model="developer" class="w-full bg-[#0A0C0F] border border-[#232936] rounded-lg p-2 text-gray-200">
                        </div>
                        <div>
                            <label class="block text-gray-400 mb-1">Publisher</label>
                            <input type="text" name="publisher" x-model="publisher" class="w-full bg-[#0A0C0F] border border-[#232936] rounded-lg p-2 text-gray-200">
                        </div>
                        <div>
                            <label class="block text-gray-400 mb-1">Año</label>
                            <input type="number" name="release_year" x-model="releaseYear" class="w-full bg-[#0A0C0F] border border-[#232936] rounded-lg p-2 text-gray-200">
                        </div>
                        <div>
                            <label class="block text-gray-400 mb-1">Región</label>
                            <input type="text" name="region" x-model="region" class="w-full bg-[#0A0C0F] border border-[#232936] rounded-lg p-2 text-gray-200">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="file_format" x-model="fileFormat" class="w-full bg-[#0A0C0F] border border-[#232936] rounded-lg p-2 text-gray-200">
                        </div>
                        <div class="sm:col-span-3">
                            ...
                            <p class="text-[10px] text-gray-500 mt-1">Escribe libremente el tamaño que deseas mostrar (Ej. 1.2 GB, 800 MB, 14.5 GB).</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-6 space-y-4">
                    ...
                    <div class="space-y-4">
                            ...
                            <div class="p-3.5 bg-[#11141A] border border-dashed border-[#232936] hover:border-blue-500/40 rounded-xl space-y-3 transition-colors">
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-bold text-gray-200">Subir / Reemplazar ROM en Cloudflare R2:</span>
                                    </div>
                                    <label class="cursor-pointer px-3 py-1.5 bg-blue-600/20 hover:bg-blue-600 text-blue-300 hover:text-white border border-blue-500/30 rounded-lg text-xs font-mono font-bold transition-all flex items-center gap-1.5 w-fit">
                                        ...
                                        <input type="file" id="romFileInputEdit" name="rom_file" @change="handleRomSelect($event)" class="hidden">
                                    </label>
                                </div>
                        ...
                        <div class="p-3.5 bg-[#0A0C0F] border border-[#232936] rounded-xl space-y-1.5">
                            ...
                            <input type="text" name="mirror_url" value="{{ old('mirror_url', $game->mirror_url) }}" placeholder="https://mirror.romhub.io/roms/juego.iso (Opcional)" class="w-full bg-[#11141A] border border-[#232936] rounded-lg p-2.5 text-xs text-gray-300 font-mono focus:border-emerald-500 focus:outline-none">
                        </div>
                ...
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-6 space-y-4">
                    <div class="flex items-center justify-between border-b border-[#232936] pb-3">
                        ...
                        <button type="button" 
                                ...
                                :disabled="seoLoading"
                                class="px-3 py-1.5 rounded-lg bg-purple-600/20 hover:bg-purple-600 text-purple-300 hover:text-white border border-purple-500/30 text-xs font-mono transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50">
                            <i data-lucide="sparkles" class="w-3.5 h-3.5 text-purple-400" x-show="!seoLoading"></i>
                            <i data-lucide="loader-2" class="w-3.5 h-3.5 animate-spin" x-show="seoLoading"></i>
                            <span x-text="seoLoading ? 'Optimizando SEO...' : '✨ Regenerar SEO con Gemini IA'"></span>
                        </button>
                    </div>
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="text" name="meta_title" x-model="metaTitle" class="w-full bg-[#0A0C0F] border border-[#232936] rounded-lg p-2.5 text-xs text-gray-200 font-sans">
                        </div>

                        <div>
                            <label class="block text-xs font-mono text-gray-400 mb-1">Meta Descripción (Snippet)</label>
                            <textarea name="meta_description" x-model="metaDescription" rows="2" class="w-full bg-[#0A0C0F] border border-[#232936] rounded-lg p-2.5 text-xs text-gray-200 font-sans"></textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="space-y-3 font-sans text-xs">
                        <div>
                            ...
                            <select name="status" class="w-full bg-[#0A0C0F] border border-[#232936] rounded-lg p-2.5 text-gray-200 font-mono">
                                <option value="PUBLISHED" {{ old('status', $game->status) === 'PUBLISHED' ? 'selected' : '' }}>PUBLICADO</option>
                                <option value="DRAFT" {{ old('status', $game->status) === 'DRAFT' ? 'selected' : '' }}>BORRADOR</option>
                                <option value="ARCHIVED" {{ old('status', $game->status) === 'ARCHIVED' ? 'selected' : '' }}>ARCHIVADO</option>
                            </select>
                        </div>

                        <div class="space-y-2.5 pt-2 border-t border-[#232936]">
                            <label class="flex items-start gap-2.5 cursor-pointer">
                                <input type="checkbox" name="is_spotlight" value="1" {{ old('is_spotlight', $game->is_spotlight) ? 'checked' : '' }} class="mt-0.5 rounded bg-[#0A0C0F] border-[#232936] text-amber-500 focus:ring-amber-500">
                                <div>
                            ...
                            <label class="flex items-start gap-2.5 cursor-pointer">
                                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $game->is_featured) ? 'checked' : '' }} class="mt-0.5 rounded bg-[#0A0C0F] border-[#232936] text-blue-600 focus:ring-blue-600">
                                <div>
                ...
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-5 space-y-3">
                    ...
                    <!-- Live Image Preview -->
                    <div x-show="coverPreview" class="relative group mx-auto w-36 aspect-[3/4.1] rounded-xl overflow-hidden border-2 border-blue-500/50 shadow-xl bg-[#0A0C0F] transition-all">
                        ...
                        <div class="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 p-2">
                            <label class="cursor-pointer px-3 py-1 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-mono text-[11px] font-bold transition-transform transform hover:scale-105">
                                ...
                                <input type="file" id="coverFileInputEdit" name="cover_image" accept="image/*" @change="handleCoverSelect($event)" class="hidden">
                            </label>
                        ...
                        <div class="absolute bottom-1.5 left-1.5 right-1.5 bg-black/85 backdrop-blur-sm px-1.5 py-0.5 rounded text-[9px] font-mono text-center text-emerald-400 truncate" x-text="coverFileName || 'Carátula Actual / Nueva'"></div>
                    </div>

                    <!-- Dropzone when no cover preview -->
                    <div x-show="!coverPreview" class="p-4 border-2 border-dashed border-[#232936] rounded-xl text-center space-y-2 hover:border-blue-500/50 transition-colors bg-[#0A0C0F]">
                        ...
                        <div class="text-xs font-sans text-gray-300">
                            <label class="cursor-pointer text-blue-400 hover:underline">
                                <span class="font-bold">Subir carátula frontal</span>
                                <input type="file" id="coverFileInputEdit" name="cover_image" accept="image/*" @change="handleCoverSelect($event)" class="hidden">
                            </label>
                        </div>
                ...
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-5 space-y-3">
                    <div class="flex items-center justify-between border-b border-[#232936] pb-2">
                        ...
                        <span class="text-[10px] font-mono text-emerald-400 bg-emerald-950/40 border border-emerald-500/20 px-2 py-0.5 rounded">{{ $game->screenshots->count() }} guardadas</span>
                    </div>
                    ...
                    <div class="p-3.5 border-2 border-dashed border-[#232936] rounded-xl text-center space-y-1.5 hover:border-blue-500/50 transition-colors bg-[#0A0C0F]">
                        ...
                        <div class="text-xs font-sans text-gray-300">
                            <label class="cursor-pointer text-blue-400 hover:underline">
                                <span class="font-bold">+ Añadir Nuevas Capturas In-Game</span>
                                <input type="file" id="screenshotFilesInputEdit" name="screenshot_files[]" accept="image/*" multiple @change="handleScreenshotsSelect($event)" class="hidden">
                            </label>
                        </div>
                        <p class="text-[10px] font-mono text-gray-500">PNG, JPG. Se convertirán a WebP HD optimizado.</p>
                    </div>

                    <!-- Existing & New Thumbnails Grid -->
                    <div class="space-y-2">
                        @if($game->screenshots->isNotEmpty())
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-48 overflow-y-auto pr-1">
                            @foreach($game->screenshots as $ss)
                            <div x-data="{ markedForDeletion: false }" 
                                 class="relative group/ss aspect-video rounded-lg overflow-hidden border border-[#232936] bg-[#0A0C0F]"
                                 :class="markedForDeletion ? 'opacity-30 border-rose-600' : ''">
                                <img src="{{ $ss->image_webp_url ?: $ss->image_url }}" class="w-full h-full object-cover">
                                <input type="checkbox" name="delete_screenshot_ids[]" value="{{ $ss->id }}" x-model="markedForDeletion" class="hidden">
                                <button type="button" 
                                        @click="markedForDeletion = !markedForDeletion" 
                                        class="absolute top-1 right-1 p-1 rounded-md text-white transition-opacity shadow-md text-[10px] font-mono cursor-pointer"
                                        :class="markedForDeletion ? 'bg-amber-600 opacity-100' : 'bg-rose-600 hover:bg-rose-500 opacity-0 group-hover/ss:opacity-100'">
                                    <span x-text="markedForDeletion ? 'Restaurar' : '✕'"></span>
                                </button>
                                <span class="absolute bottom-1 left-1 px-1 rounded bg-black/80 text-[8px] font-mono text-emerald-400">Guardada</span>
                            </div>
                            @endforeach
                        </div>
                        @endif
                        ...
                        <div x-show="screenshotPreviews.length > 0" x-cloak class="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1 max-h-48 overflow-y-auto pr-1 border-t border-[#232936]">
                            <template x-for="(item, idx) in screenshotPreviews" :key="idx">
                                <div class="relative group/ss aspect-video rounded-lg overflow-hidden border-2 border-blue-500/50 bg-[#0A0C0F]">
                                    ...
                                    <span class="absolute bottom-1 left-1 px-1 rounded bg-blue-900/80 text-[8px] font-mono text-white">Nueva #<span x-text="idx + 1"></span></span>
                                </div>
                            </template>
                        </div>
                    </div>
                ...
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-5 space-y-3">
                    ...
                    <div class="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                        @php $gameBadgeIds = $game->badges->pluck('id')->toArray(); @endphp
                        ...
                        <label class="flex items-center justify-between p-2 rounded-lg bg-[#0A0C0F] border border-[#232936] cursor-pointer hover:border-blue-500/40">
                            ...
                            <input type="checkbox" name="badge_ids[]" value="{{ $badge->id }}" {{ in_array($badge->id, $gameBadgeIds) ? 'checked' : '' }} class="rounded bg-[#11141A] border-[#232936] text-blue-600">
                        </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-5 space-y-3">
                    ...
                    @php $gameCatIds = $game->categories->pluck('id')->toArray(); @endphp
                    <div class="grid grid-cols-2 gap-1.5 max-h-48 overflow-y-auto pr-1">
                        ...
                        <label class="flex items-center gap-2 p-2 rounded-lg bg-[#0A0C0F] border border-[#232936] text-xs font-mono text-gray-300 cursor-pointer hover:border-blue-500/40">
                            <input type="checkbox" name="category_ids[]" value="{{ $category->id }}" {{ in_array($category->id, $gameCatIds) ? 'checked' : '' }} class="rounded bg-[#11141A] border-[#232936] text-blue-600">
                ...
                <div class="bg-[#11141A] border border-[#232936] rounded-2xl p-5 space-y-3">
                    ...
                    @php $gameFranchiseIds = $game->franchises->pluck('id')->toArray(); @endphp
                    <div class="grid grid-cols-2 gap-1.5 max-h-48 overflow-y-auto pr-1">
                        ...
                        <label class="flex items-center gap-2 p-2 rounded-lg bg-[#0A0C0F] border border-[#232936] text-xs font-mono text-gray-300 cursor-pointer hover:border-amber-500/40">
                            <input type="checkbox" name="franchise_ids[]" value="{{ $franchise->id }}" {{ in_array($franchise->id, $gameFranchiseIds) ? 'checked' : '' }} class="rounded bg-[#11141A] border-[#232936] text-amber-500">
                ...
                <button type="submit" class="w-full py-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-black text-xs uppercase tracking-wider transition-all shadow-xl shadow-blue-600/30 flex items-center justify-center gap-2">
                    <i data-lucide="save" class="w-4 h-4"></i> Actualizar Videojuego
                </button>
...
function gameEditForm() {
    return {
        title: @json(old('title', $game->title)),
        consoleName: '',
        description: @json(old('description', $game->description ?? '')),
        metaTitle: @json(old('meta_title', $game->meta_title ?? '')),
        metaDescription: @json(old('meta_description', $game->meta_description ?? '')),
        developer: @json(old('developer', $game->developer ?? '')),
        ...
        mirrors: @json(old('mirrors', $game->download_links ?: [])),
        ...
        activeRomXhr: null,
        coverPreview: @json($game->cover_url ?: null),
        coverFileName: @json($game->cover_url ? 'Carátula Actual Guardada' : ''),
        bannerPreview: @json($game->banner_url ?: null),
        bannerFileName: @json($game->banner_url ? 'Banner Actual Guardado' : ''),
        screenshotPreviews: [],
        aiLoading: false,
        seoLoading: false,
        descLoading: false,
        ...
        clearCoverPreview() {
            ...
            this.coverFileName = '';
            let input = document.getElementById('coverFileInputEdit');
        ...
        clearBannerPreview() {
            ...
            this.bannerFileName = '';
            let input = document.getElementById('bannerFileInputEdit');
            if (input) input.value = '';
            this.$nextTick(() => { if (window.lucide) { lucide.createIcons(); } });
        },
        ...
        renderMarkdown(text) {
            if (!text || !text.trim()) {
                return '<p class="text-gray-500 italic p-4 text-center">Sinopsis vacía. Escribe o pulsa "Regenerar con IA" para crear ~200 palabras...</p>';
            }
            if (window.marked) {
        ...
        uploadRomDirectly() {
            let fileInput = document.getElementById('romFileInputEdit');
